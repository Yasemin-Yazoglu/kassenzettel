...
export default function SpendOverTimeChart({ expenses }: Props) {
    const [period, setPeriod] = useState<Period>("month");
    const [anchor, setAnchor] = useState<Date>(new Date());

    function handlePeriodChange(newPeriod: Period) {
        setPeriod(newPeriod);
        setAnchor(new Date()); // jump back to "now" whenever the granularity changes
    }
    ...
    return (
        <div className="rounded-2xl bg-white/5 border border-white/10 p-5 flex flex-col gap-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <h3 className="text-white font-semibold">Ausgaben im Verlauf</h3>
            ...
            <Graph
                ...
                emptyMessage="Noch keine Ausgaben in diesem Zeitraum."
            />
        </div>
    );
}